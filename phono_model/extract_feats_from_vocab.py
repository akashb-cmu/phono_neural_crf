...
arg_parser.add_argument("-SV", "--src_vocab_file_prefix", help="Name prefix for vocabulary files with all source language"
                                                            "words for which features need to be extract", default=None,
                        type=str)
arg_parser.add_argument("-SCD", "--src_conll_dir", help="Directory that contains the source language conll files",
...
arg_parser.add_argument("-T", "--trg_lang",
                        help="Name of the target language", default=None,
                        type=str)
arg_parser.add_argument("-TV", "--trg_vocab_file_prefix",
                        help="Name prefix for vocabulary files with all target language words for which features need "
                             "to be extract", default=None,
                        type=str)
arg_parser.add_argument("-TCD", "--trg_conll_dir", help="Directory that contains the target language conll files",
                        default=None, type=str)
arg_parser.add_argument("-TD", "--trg_vocab_dir", help="Directory that contains the target language vocabulary files",
                        default=None, type=str)
...
arg_parser.add_argument("-TS", "--trg_lang_space", help="Epitran language space code for the target language",
                        ...
                        type=str)
arg_parser.add_argument("-op_dir", "--output_dir", help="Directory where the output feat files must be generated",
                        type=str)
...
src_conll_dir = args.src_conll_dir
...
trg_conll_dir = args.trg_conll_dir

op_dir = args.output_dir

# Args sanity check
if src_vocab_dir is None:
    assert src_conll_dir is not None, "Neither vocab dir nor conll dir provided for source lang. Provide at least one (vocab dir takes precedence"

if trg_lang is not None and trg_vocab_dir is None:
    assert trg_conll_dir is not None, "Neither vocab dir nor conll dir provided for trg lang. Provide at least one (vocab dir takes " \
                                      "precedence"
...
def get_vocab(conll_dir):
    vocab = set()
    for [path, dirs, files] in os.walk(conll_dir):
        for file in files:
            if file.endswith(".conll"):
                with codecs.open(os.path.join(path, file), "r", encoding="utf-8") as ip_file:
                    for line in ip_file:
                        line = line.strip(" \t\r\n")
                        if len(line) > 0:
                            word = line.split()[0]
...
if src_vocab_dir:
    print("Reading pre-extracted vocabs")
    src_vocab = read_all_vocabs(src_vocab_dir, src_vocab_file_prefix)
else:
    print("Creating vocabs from conll")
    src_vocab = get_vocab(src_conll_dir)

print("Src vocab len = %d"%(len(src_vocab)))
...
if trg_lang:
    if trg_vocab_dir:
        ...
        print("Creating vocabs from conll")
        trg_vocab = get_vocab(trg_conll_dir)
    print("Trg vocab len = %d" % (len(trg_vocab)))
...
SRC_IN_SRC_SPACE = os.path.join(op_dir, src_lang + "_in_" + src_lang +"_ipa_")
...
trg_ortho_char_to_id  = get_phono_vecs(trg_vocab, trg_epi, word_categories)
    ...
    for key in trg_phono_char_to_id.keys():
        ...
        if src_phono_char_to_id[key] != trg_phono_char_to_id[key]:
            print(src_phono_char_to_id[key], trg_phono_char_to_id[key])
    TRG_IN_TRG_SPACE = os.path.join(op_dir, trg_lang + "_in_" + trg_lang + "_ipa_")
...
if trg_lang:
    ...
    SRC_IN_COMBO_SPACE = os.path.join(op_dir,src_lang + "_in_combo_unified_ipa_")
    TRG_IN_COMBO_SPACE = os.path.join(op_dir,trg_lang + "_in_combo_unified_ipa_")
    ...
    SRC_IN_TRG_SPACE = os.path.join(op_dir,src_lang + "_in_" + trg_lang  +"_unified_ipa_")
    TRG_IN_TRG_SPACE = os.path.join(op_dir,trg_lang + "_in_" + trg_lang  + "_unified_ipa_")
    ...
    SRC_IN_SRC_SPACE = os.path.join(op_dir,src_lang + "_in_" + src_lang  + "_unified_ipa_")
    TRG_IN_SRC_SPACE = os.path.join(op_dir,trg_lang + "_in_" + src_lang  + "_unified_ipa_")
